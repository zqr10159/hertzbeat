...
class GlobalExceptionHandlerObservabilityQueryTest {
    ...
    @Test
    void mapsTimeoutToServiceUnavailableWithRetryHint() throws Exception {
        mockMvc.perform(get("/test/query/timed-out"))
                .andExpect(status().isServiceUnavailable())
                .andExpect(header().string("Retry-After", "1"))
                .andExpect(jsonPath("$.msg").value("Observability traces query timed out."));
    }
    ...
    private static class FailingQueryController {
        ...
        @GetMapping("/test/query/timed-out")
        void timedOut() {
            throw new ObservabilityQueryAdmissionException(
                    "traces", ObservabilityQueryAdmissionException.Reason.TIMED_OUT, null);
        }
    }
}
