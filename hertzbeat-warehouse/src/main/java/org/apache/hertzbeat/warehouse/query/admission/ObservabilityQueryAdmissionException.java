...
/**
 * Signals an overloaded, timed out, or cancelled observability query.
 */
public class ObservabilityQueryAdmissionException extends RuntimeException {
    ...
    private static String message(String signal, Reason reason) {
        return switch (reason) {
            ...
            case TIMED_OUT -> "Observability " + signal + " query timed out.";
            case CANCELLED -> "Observability " + signal + " query was cancelled.";
        };
    }

    /**
     * Admission failure classification used by the HTTP exception boundary.
     */
    public enum Reason {
        OVERLOADED,
        TIMED_OUT,
