...
package org.apache.hertzbeat.collector.core.collect.modbus;
...
public class ModbusCollectTest {
    ...
    void preCheck() {
        ...
        assertDoesNotThrow(() -> {
            ...
            metrics.setModbus(modbus); // Updates the reference in metrics
            metrics.setAliasFields(List.of(
                    "responseTime",
                    "holding-register:0",
                    "holding-register:1",
    ...
    void collect() {
        ...
        assertDoesNotThrow(() -> {
            ...
            // preCheck will copy modbus params to plc params
            modbusCollect.preCheck(metrics);
            
            // Ensure plc host is set if preCheck didn't do it (though it should)
            if (metrics.getPlc() != null && metrics.getPlc().getHost() == null) {
                metrics.getPlc().setHost(modbus.getHost());
            }
        ...
        assertDoesNotThrow(() -> {
            ...
            modbusCollect.preCheck(metrics);
            if (metrics.getPlc().getHost() == null) {
                metrics.getPlc().setHost(modbus.getHost());
            }
            modbusCollect.collect(builder, metrics);
        });
